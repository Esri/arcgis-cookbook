...
arcgis_enterprise_webadaptor 'Uninstall Web Adaptor for Notebook Server' do
  install_dir node['arcgis']['web_adaptor']['install_dir']
  instance_name node['arcgis']['notebook_server']['wa_name']
  run_as_user node['arcgis']['run_as_user']
  if node['platform'] == 'windows'
    product_code Utils.wa_product_code(node['arcgis']['notebook_server']['wa_name'],
                                       node['arcgis']['web_adaptor']['product_codes'])
    only_if {
      !Utils.wa_product_code(node['arcgis']['notebook_server']['wa_name'],
                             node['arcgis']['web_adaptor']['product_codes']).nil?
    }
  else
    only_if {
      ::File.exist?(::File.join(node['arcgis']['web_server']['webapp_dir'],
                                node['arcgis']['notebook_server']['wa_name'] + '.war'))
    }
  end
  action :uninstall
end
