...
module ArcGIS
  #
  # Client class for ArcGIS Video Server administrative directory API.
  #
  class VideoServerAdminClient
    MAX_RETRIES = 300
    SLEEP_TIME = 10.0
    ...
    def upgrade_required?
      ...
      if response.code.to_i == 200
        return JSON.parse(response.body)['upgradeStatus'] == 'UPGRADE_REQUIRED' ||
               ...
               JSON.parse(response.body)['isUpgrade'] # Video Server
      end
    ...
    def create_site(server_directories_root,
      ...
      directories = [{
        'path' => ::File.join(server_directories_root, 'directories', 'arcgisvideouploads'),
        'name' => 'arcgisvideouploads',
        'id' => SecureRandom.uuid,
        'type' => 'UPLOADS'
      }, {
        'path' => ::File.join(server_directories_root, 'directories', 'arcgisvideoservices'),
    ...
    def set_server_ssl_certificate(machine_name, cert_alias)
      ...
      request.set_form_data(
        ...
        'numVideoProcessors' => machine['numVideoProcessors'],
