...
if node['platform'] == 'windows'
  arcgis_enterprise_server 'Couple ArcGIS Server with Active Directory' do
    ...
    action :set_identity_store_to_asp_net
  end
  
  arcgis_enterprise_server 'Assign ArcGIS Server roles to Active Directory groups' do
    server_url node['arcgis']['server']['url']
    username node['arcgis']['server']['admin_username']
    password node['arcgis']['server']['admin_password']
