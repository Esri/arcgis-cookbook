...
* `node['arcgis']['cloud']['provider']` = Cloud provider. Default value is set to `ec2` if arcgis_cloud_platform environment variable is set to `aws` and to `node['cloud']['provider']` attribute value otherwise.
...
* `node['arcgis']['server']['use_join_site_tool']` = If set to true, the join-site command line tool is used to add machine to the site specified for the 'primary_server_url' and the default cluster. Default value is `false`.
...
* `node['arcgis']['server']['config_store_connection_string']` = ArcGIS Server configuration store type. Default value is `C:\arcgisserver\config-store` on Windows and `/arcgis/server/usr/config-store` on Linux.
...
* `node['arcgis']['portal']['data_dir']` = Data directory path used by Portal for ArcGIS setup. The path must be a local directory, not a shared network directory. Dafault path on Windows is `C:\arcgisportal`; on Linux, it is `/arcgis/portal/usr/arcgisportal/`.
...
* `node['arcgis']['portal']['authorization_file']` = Portal for ArcGIS authorization file path. Default location and file name is `C:\Temp\portal_license.json` on Windows and `/tmp/portal_license.json` on Linux.
* `node['arcgis']['portal']['authorization_file_version']` = Portal for ArcGIS authorization file version. Default value is `10.9`.
...
Resets the function of the federated server in the ArcGIS Enterprise portal.
...
Enables file sharing via a DNS Alias
...
Registers ArcGIS Server with GeoAnalytics function to the ArcGIS Enterprise portal.

### arcgis-enterprise::enable_imagehosting

Registers ArcGIS Server with ImageHosting function to the ArcGIS Enterprise portal.

### arcgis-enterprise::enable_rasteranalytics

Registers ArcGIS Server with RasterAnalytics function to the ArcGIS Enterprise portal.

### arcgis-enterprise::federation

Registers and federates ArcGIS Server with the ArcGIS Enterprise portal.

### arcgis-enterprise::fileserver

Creates and shares directories on the file server machine.
...
Installs and configures Portal for ArcGIS on a standby machine.
...
Unfederates an ArcGIS Server site from the ArcGIS Enterprise portal.
...
Unregisters all unavailable server machines in the 'default' cluster from the ArcGIS Server site.
