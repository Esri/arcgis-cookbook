...
This workflow demonstrates how to upgrade a [base ArcGIS Enterprise](https://enterprise.arcgis.com/en/get-started/latest/windows/base-arcgis-enterprise-deployment.htm) 10.9 - 12.0 on a single machine deployed using the [arcgis-enterprise-base]({{ site.baseurl }}/templates/arcgis-enterprise-base/12.0.html) deployment template to version 12.1.
...
* Authorization files for Portal for ArcGIS 12.1 and ArcGIS Server 12.1.
* The original JSON files used for the initial deployment or the last upgrade.

## Step 1. Upgrade Cinc Client

Upgrade [Cinc Client](https://cinc.sh/start/client/) to version 18 recommended for Chef Cookbooks for ArcGIS 5.4.0.
...
## Step 2. Download Chef Cookbooks for ArcGIS v5.4.0

### On Windows

Download the [arcgis-5.4.0-cookbooks.zip](https://github.com/Esri/arcgis-cookbook/releases/tag/v5.4.0) archive to the machine.

Delete `C:\cinc\cookbooks` and `C:\cinc\templates` directories and extract the contents of the arcgis-5.4.0-cookbooks.zip archive to `C:\cinc`.
...
## Step 3. Download ArcGIS Enterprise 12.1 Setups
...
> Note: If you don't have an ArcGIS Online user account or already have all the setup archives required by the [arcgis-enterprise-base]({{ site.baseurl }}/templates/arcgis-enterprise-base/12.1) 12.1 deployment template in a local or shared directory, then you can skip Step 3 and specify the directory path using the arcgis.repository.archives attribute in the arcgis-enterprise-primary.json file used in Step 4.

### On Windows

Copy file `C:\cinc\templates\arcgis-enterprise-base\12.1\windows\arcgis-enterprise-files.json` to `C:\cinc`.
...
Copy file `/opt/cinc/templates/arcgis-enterprise-base/12.1/linux/arcgis-enterprise-files.json` to `/opt/cinc`.
...
Use the copy_attributes tool to copy attributes from the original `original\arcgis-enterprise-primary.json` JSON file used for the initial deployment to the `arcgis-enterprise-primary.json` file of the 12.1 arcgis-enterprise-base template.
...
Update authorization file paths for Portal for ArcGIS 12.1 and ArcGIS Server 12.1 in `arcgis-enterprise-primary.json`:

* arcgis.portal.authorization_file - Change to Portal for ArcGIS 12.1 authorization file path.
* arcgis.server.authorization_file - Change to ArcGIS Server 12.1 authorization file path.
...
Copy file `/opt/cinc/templates/arcgis-enterprise-base/12.1/linux/arcgis-enterprise-primary.json` to `/opt/cinc`.
