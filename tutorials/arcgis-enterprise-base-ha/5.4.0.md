...
This workflow demonstrates how to deploy a [highly available ArcGIS Enterprise](https://enterprise.arcgis.com/en/portal/latest/administer/windows/configure-highly-available-system.htm) 12.1 base deployment across three machines:
...
* ArcGIS Enterprise 12.1 setups from [My Esri](https://my.esri.com).
* ArcGIS Enterprise 12.1 authorization files. Make sure to use the correct authorization file for different products. 
...
On all of the machines, install [Cinc Client](https://cinc.sh/start/client/) version 18 recommended for Chef Cookbooks for ArcGIS 5.4.0.
...
On all of the machines, download and extract Chef Cookbooks for ArcGIS 5.4.0.

### On Windows

Download the [arcgis-5.4.0-cookbooks.zip](https://github.com/Esri/arcgis-cookbook/releases/tag/v5.4.0) archive to the machine.

Extract the contents of the arcgis-5.4.0-cookbooks.zip archive to `C:\cinc`.

### On Linux

Download the [arcgis-5.4.0-cookbooks.tar.gz](https://github.com/Esri/arcgis-cookbook/releases/tag/v5.4.0) archive to the machine.

Delete `/opt/cinc/cookbooks` and `/opt/cinc/templates` directories and extract the contents of the arcgis-5.4.0-cookbooks.tar.gz archive to `/opt/cinc`.
...
Copy file `C:\cinc\templates\arcgis-enterprise-base\12.1\windows\arcgis-enterprise-fileserver.json` to `C:\cinc`.
...
- `\\<File Server Machine hostname>\arcgisbackup`
- `\\<File Server Machine hostname>\arcgisportal`
- `\\<File Server Machine hostname>\arcgisserver`

### On Linux

Copy file `/opt/cinc/templates/arcgis-enterprise-base/12.1/linux/arcgis-enterprise-fileserver.json` to `/opt/cinc`.
...
- `/net/<File Server Machine hostname>/gisdata/arcgisbackup`
- `/net/<File Server Machine hostname>/gisdata/arcgisportal`
...
Create directory `C:\Software\Archives` and copy the ArcGIS Enterprise 12.1 setup archives to that directory:
...
Create directory `C:\Software\AuthorizationFiles\12.1` and copy the software authorization files for 12.1 ArcGIS Server and Portal for ArcGIS to that directory.

Create directory `C:\Software\Certificates` and copy the SSL certificate to that directory.

### On Linux

Create directory `/opt/software/archives` and copy the following ArcGIS Enterprise 12.1 setup archives to that directory:
...
* [apache-tomcat-10.1.41.tar.gz](https://archive.apache.org/dist/tomcat/tomcat-10/v10.1.41/bin/apache-tomcat-10.1.41.tar.gz) (will be downloaded from the internet if not present in the local ArcGIS software repository)
* [OpenJDK17U-jdk_x64_linux_hotspot_17.0.15_6.tar.gz](https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.15%2B6/OpenJDK17U-jdk_x64_linux_hotspot_17.0.15_6.tar.gz) (will be downloaded from the internet if not present in the local ArcGIS software repository)

Create directory `/opt/software/authorization_files/12.1` and copy the software authorization files for 12.1 ArcGIS Server and Portal for ArcGIS to that directory.
...
Copy file `C:\cinc\templates\arcgis-enterprise-base\12.1\windows\arcgis-enterprise-primary.json` to `C:\cinc`.
...
Copy the file `/opt/cinc/templates/arcgis-enterprise-base/12.1/linux/arcgis-enterprise-primary.json` to `/opt/cinc`.
...
Copy file `C:\cinc\templates\arcgis-enterprise-base\12.1\windows\arcgis-enterprise-standby.json` to `C:\cinc`.
...
Copy the file `/opt/cinc/templates/arcgis-enterprise-base/12.1/linux/arcgis-enterprise-standby.json` to `/opt/cinc`.
