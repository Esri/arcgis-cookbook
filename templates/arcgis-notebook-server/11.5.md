...
Consult the ArcGIS Notebook Server 11.5 system requirements documentation for the required/recommended hardware specification.
...
To upgrade an ArcGIS Notebook Server site deployed using the arcgis-notebook-server deployment template to the 11.5 version, you will need:

* ArcGIS Notebook Server 11.5 setup archive,
* ArcGIS Notebook Server 11.5 Docker container images,
* ArcGIS Web Adaptor 11.5 setup archive, if Web Adaptors were installed in the initial deployment,
* ArcGIS Notebook Server 11.5 software authorization file,
...
Upgrading ArcGIS Notebook Server deployments to 11.5 requires upgrading all ArcGIS Notebook Server machines.

> Note that in 11.5, ArcGIS Web Adaptor is installed using arcgis-webadaptor deployment template.

1. Upgrade the first ArcGIS Notebook Server machine.
  
   Copy attributes from the original `notebook-server-primary.json` JSON file created from a 10.7.1-10.8.1 arcgis-notebook-server template to the `notebook-server.json` file of the 11.5 arcgis-notebook-server template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/notebook-server-primary.json <arcgis-notebook-server 11.5 template>/notebook-server.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis--notebook-server 11.5 template>/notebook-server.json
   ```
...
2. Upgrade the additional ArcGIS Notebook Server machines.
   ...
   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/notebook-server-node.json <arcgis-notebook-server 11.5 template>/notebook-server-node.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-notebook-server 11.5 template>/notebook-server-node.json
   ```

3. Upgrade ArcGIS Web Adaptor used with ArcGIS Notebook Server.

   Copy attributes from the original `notebook-server-primary.json` JSON file created from a 10.7.1-10.8.1 arcgis-notebook-server template to the `arcgis-notebook-server-webadaptor.json` file of the 11.5 arcgis-webadaptor template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/notebook-server-primary.json <arcgis-webadaptor 11.5 template>/arcgis-notebook-server-webadaptor.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-webadaptor 11.5 template>/arcgis-notebook-server-webadaptor.json
   ```

### Upgrade from 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4

Upgrading ArcGIS Notebook Server deployments from 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 to 11.5 requires upgrading all ArcGIS Notebook Server machines. The file server machine does not require any changes.

1. Upgrade the first ArcGIS Notebook Server machine.
  
   Copy attributes from the original `notebook-server.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 arcgis-notebook-server template to the `notebook-server.json` file of the 11.5 arcgis-notebook-server template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/notebook-server.json <arcgis-notebook-server 11.5 template>/notebook-server.json
   ```
...
2. Upgrade the additional ArcGIS Notebook Server machines.
   ...
   Copy attributes from the original `notebook-server-node.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 arcgis-notebook-server template to the `notebook-server-node.json` file of the 11.5 arcgis-notebook-server template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/notebook-server-node.json <arcgis-server 11.5 template>/notebook-server-node.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-server 11.5 template>/notebook-server-node.json
   ```

3. Upgrade ArcGIS Notebook Server Web Adaptors.

   Copy attributes from the original `notebook-server-webadaptor.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 arcgis-notebook-server template to the `arcgis-notebook-server-webadaptor.json` file of the 11.5 arcgis-webadaptor template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/notebook-server-webadaptor.json <arcgis-webadaptor 11.5 template>/arcgis--notebook-server-webadaptor.json
   ```
