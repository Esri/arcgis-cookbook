...
Consult the ArcGIS Mission Server 11.5 system requirements documentation for the required/recommended hardware specification.
...
To upgrade an ArcGIS Mission Server site deployed using the arcgis-mission-server deployment template to the 11.5 version, you will need:

* ArcGIS Mission Server 11.5 setup archive,
* ArcGIS Web Adaptor 11.5 setup archive, if Web Adaptors were installed in the initial deployment,
* ArcGIS Mission Server 11.5 software authorization file,
...
Upgrading ArcGIS Mission Server deployments to 11.5 requires upgrading all ArcGIS Mission Server machines.

> Note that in 11.5, ArcGIS Web Adaptor is installed using the new arcgis-webadaptor deployment template.

1. Upgrade the first ArcGIS Mission Server machine.
  
   Copy attributes from the original `mission-server-primary.json` JSON file created from the 10.8/10.8.1 arcgis-mission-server template to the `mission-server.json` file of the 11.5 arcgis-mission-server template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/mission-server-primary.json <arcgis-mission-server 11.5 template>/mission-server.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-mission-server 11.5 template>/mission-server.json
   ```

2. Upgrade the ArcGIS Web Adaptor for the ArcGIS Mission Server site.

   Copy attributes from the original `mission-server-primary.json` JSON file created from the 10.8/10.8.1 arcgis-mission-server template to the `arcgis-mission-server-webadaptor.json` file of the 11.5 arcgis-webadaptor template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/mission-server-primary.json <arcgis-webadaptor 11.5 template>/arcgis-mission-server-webadaptor.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-webadaptor 11.5 template>/arcgis-mission-server-webadaptor.json
   ```

### Upgrade from 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4

Upgrading ArcGIS Mission Server deployments from 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 to 11.5 requires upgrading all ArcGIS Mission Server machines. The file server machine does not require any changes.

1. Upgrade the first ArcGIS Mission Server machine.
  
   Copy attributes from the original `mission-server.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 arcgis-mission-server template to the `mission-server.json` file of the 11.5 arcgis-mission-server template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/mission-server.json <arcgis-mission-server 11.5 template>/mission-server.json
   ```
...
2. Upgrade additional ArcGIS Mission Server machines.
   ...
   Copy attributes from the original `mission-server-node.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2 or 11.3 arcgis-mission-server template to the `mission-server-node.json` file of the 11.5 arcgis-mission-server template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/mission-server-node.json <arcgis-server 11.5 template>/mission-server-node.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-server 11.5 template>/mission-server-node.json
   ```

3. Upgrade ArcGIS Web Adaptor for the ArcGIS Mission Server site.

   Copy attributes from the original `mission-server-webadaptor.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 arcgis-mission-server template to the `arcgis-mission-server-webadaptor.json` file of the 11.5 arcgis-webadaptor template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/mission-server-webadaptor.json <arcgis-webadaptor 11.5 template>/arcgis-mission-server-webadaptor.json
   ```
