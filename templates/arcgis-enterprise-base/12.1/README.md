...
* ArcGIS Data Store configured as relational and object store
...
Consult the ArcGIS Enterprise 12.1 system requirements documentation for the required/recommended hardware specification.
...
* ArcGIS_Server_Windows_121_200157.exe
* ArcGIS_Server_Windows_121_200157.exe.001
* ArcGIS_Web_Adaptor_for_Microsoft_IIS_121_200166.exe
* Portal_for_ArcGIS_Windows_121_200161.exe
* Portal_for_ArcGIS_Windows_121_200161.exe.001
* Portal_for_ArcGIS_Web_Styles_Windows_121_200163.exe
* dotnet-hosting-10.0.1-win.exe (Will be downloaded from the internet if the file is not present)
...
* ArcGIS_Server_Linux_121_200205.tar.gz
...
* apache-tomcat-10.1.41.tar.gz (Alternatively, if it is not present in the local ArcGIS software repository, you can remove the tomcat.tarball_path attribute from the json and it will be downloaded from the internet for you.)
* OpenJDK17U-jdk_x64_linux_hotspot_17.0.15_6.tar.gz (Alternatively, if it is not present in the local ArcGIS software repository, you can remove the java.tarball_path attribute from the json and it will be downloaded from the internet for you.)
...
To register a cloud native object store such as Amazon S3 bucket with ArcGIS Server as managed object store, use arcgis-enterprise-cloud-object-store.json file.
...
To upgrade a base ArcGIS Enterprise deployed using the arcgis-enterprise-base deployment template to the 12.1 version, you will need:

* ArcGIS 12.1 setup archives,
* ArcGIS 12.1 software authorization files,
...
Upgrading base ArcGIS Enterprise deployments from 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, 11.5, or 12.0 to 12.1 requires upgrading the primary and standby machines. The file server machine does not require any changes. Steps 1 and 3 are not required for single-machine deployments.

1. Begin the upgrade on the standby machine.

   Copy attributes from the original `arcgis-enterprise-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, 11.5, or 12.0 arcgis-enterprise-base template to the `arcgis-enterprise-install.json` file of the 12.1 arcgis-enterprise-base template and `arcgis-server-webadaptor-unregister.json` file of the 12.1 arcgis-webadaptor template.

   > Tile Cache ArcGIS Data Store needs to be migrated to object store prior to upgrading to 12.0.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <12.1 JSON templates>/arcgis-enterprise-install.json
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <12.1 JSON templates>/arcgis-server-webadaptor-unregister.json
   ```
   ...
   ```shell
   chef-client -z -j <12.1 JSON templates>/arcgis-server-webadaptor-unregister.json
   ```
   ...
   ```shell
   chef-client -z -j <12.1 JSON templates>/arcgis-enterprise-install.json
   ```

2. Upgrade on the primary machine.

   Copy attributes from the original `arcgis-enterprise-primary.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, 11.5, or 12.0 arcgis-enterprise-base template to the `arcgis-enterprise-primary.json` file of the 12.1 arcgis-enterprise-base template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-primary.json <12.1 JSON templates>/arcgis-enterprise-primary.json
   ```
   ...
   ```shell
   chef-client -z -j <12.1 JSON templates>/arcgis-enterprise-primary.json
   ```

3. Complete upgrading on the standby machine.

   Copy attributes from the original `arcgis-enterprise-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, 11.5, or 12.0 arcgis-enterprise-base template to the `arcgis-enterprise-standby.json` file of the 12.1 arcgis-enterprise-base template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <12.1 JSON templates>/arcgis-enterprise-standby.json
   ```
   ...
   ```shell
   chef-client -z -j <12.1 JSON templates>/arcgis-enterprise-standby.json
   ```
