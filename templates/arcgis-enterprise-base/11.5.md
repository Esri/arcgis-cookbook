...
Consult the ArcGIS Enterprise 11.5 system requirements documentation for the required/recommended hardware specification.
...
* apache-tomcat-9.0.97.tar.gz (Alternatively, if it is not present in the local ArcGIS software repository, you can remove the tomcat.tarball_path attribute from the json and it will be downloaded from the internet for you.)
* OpenJDK17U-jdk_x64_linux_hotspot_17.0.13_11.tar.gz (Alternatively, if it is not present in the local ArcGIS software repository, you can remove the java.tarball_path attribute from the json and it will be downloaded from the internet for you.)
...
To upgrade a base ArcGIS Enterprise deployed using the arcgis-enterprise-base deployment template to the 11.5 version, you will need:

* ArcGIS 11.5 setup archives,
* ArcGIS 11.5 software authorization files,
...
Upgrading base ArcGIS Enterprise deployments from 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 to 11.5 requires upgrading the primary and standby machines. The file server machine does not require any changes. Steps 1 and 3 are not required for single-machine deployments.

1. Begin the upgrade on the standby machine.

   Copy attributes from the original `arcgis-enterprise-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 arcgis-enterprise-base template to the `arcgis-enterprise-install.json` file of the 11.5 arcgis-enterprise-base template and `arcgis-server-webadaptor-unregister.json` file of the 11.5 arcgis-webadaptor template.

   > TileCache ArcGIS Data Store is not installed and configured by 11.5 arcgis-enterprise-base template by default. To upgrade TileCache Data Store to 11.5, set the values of arcgis.data_store.types, arcgis.data_store.setup_options, and arcgis.data_store.tilecache attributes to the values from the original JSON file.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <11.5 JSON templates>/arcgis-enterprise-install.json
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <11.5 JSON templates>/arcgis-server-webadaptor-unregister.json
   ```
   ...
   ```shell
   chef-client -z -j <11.5 JSON templates>/arcgis-server-webadaptor-unregister.json
   ```
   ...
   ```shell
   chef-client -z -j <11.5 JSON templates>/arcgis-enterprise-install.json
   ```

2. Upgrade on the primary machine.

   Copy attributes from the original `arcgis-enterprise-primary.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, or 11.4 arcgis-enterprise-base template to the `arcgis-enterprise-primary.json` file of the 11.5 arcgis-enterprise-base template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-primary.json <11.5 JSON templates>/arcgis-enterprise-primary.json
   ```
   ...
   ```shell
   chef-client -z -j <11.5 JSON templates>/arcgis-enterprise-primary.json
   ```

3. Complete upgrading on the standby machine.

   Copy attributes from the original `arcgis-enterprise-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 arcgis-enterprise-base template to the `arcgis-enterprise-standby.json` file of the 11.5 arcgis-enterprise-base template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <11.5 JSON templates>/arcgis-enterprise-standby.json
   ```
   ...
   ```shell
   chef-client -z -j <11.5 JSON templates>/arcgis-enterprise-standby.json
   ```
