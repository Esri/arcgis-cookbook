...
Consult the ArcGIS Enterprise 12.0 system requirements documentation for the required/recommended hardware specification.
...
* Portal_for_ArcGIS_Windows_120_197706.exe.001
* Portal_for_ArcGIS_Web_Styles_Windows_120_197708.exe
* dotnet-hosting-8.0.11-win.exe (Will be downloaded from the internet if the file is not present)
...
To upgrade a base ArcGIS Enterprise deployed using the arcgis-enterprise-base deployment template to the 12.0 version, you will need:

* ArcGIS 12.0 setup archives,
* ArcGIS 12.0 software authorization files,
...
Upgrading base ArcGIS Enterprise deployments from 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, or 11.5 to 12.0 requires upgrading the primary and standby machines. The file server machine does not require any changes. Steps 1 and 3 are not required for single-machine deployments.

1. Begin the upgrade on the standby machine.

   Copy attributes from the original `arcgis-enterprise-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, or 11.5 arcgis-enterprise-base template to the `arcgis-enterprise-install.json` file of the 12.0 arcgis-enterprise-base template and `arcgis-server-webadaptor-unregister.json` file of the 12.0 arcgis-webadaptor template.

   > TileCache ArcGIS Data Store is not installed and configured by 12.0 arcgis-enterprise-base template by default. To upgrade TileCache Data Store to 12.0, set the values of arcgis.data_store.types, arcgis.data_store.setup_options, and arcgis.data_store.tilecache attributes to the values from the original JSON file.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <12.0 JSON templates>/arcgis-enterprise-install.json
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <12.0 JSON templates>/arcgis-server-webadaptor-unregister.json
   ```
   ...
   ```shell
   chef-client -z -j <12.0 JSON templates>/arcgis-server-webadaptor-unregister.json
   ```
   ...
   ```shell
   chef-client -z -j <12.0 JSON templates>/arcgis-enterprise-install.json
   ```

2. Upgrade on the primary machine.

   Copy attributes from the original `arcgis-enterprise-primary.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, or 11.5 arcgis-enterprise-base template to the `arcgis-enterprise-primary.json` file of the 12.0 arcgis-enterprise-base template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-primary.json <12.0 JSON templates>/arcgis-enterprise-primary.json
   ```
   ...
   ```shell
   chef-client -z -j <12.0 JSON templates>/arcgis-enterprise-primary.json
   ```

3. Complete upgrading on the standby machine.

   Copy attributes from the original `arcgis-enterprise-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, or 11.5 arcgis-enterprise-base template to the `arcgis-enterprise-standby.json` file of the 12.0 arcgis-enterprise-base template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <12.0 JSON templates>/arcgis-enterprise-standby.json
   ```
   ...
   ```shell
   chef-client -z -j <12.0 JSON templates>/arcgis-enterprise-standby.json
   ```
