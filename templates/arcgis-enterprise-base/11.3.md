...
Consult the ArcGIS Enterprise 11.3 system requirements documentation for the required/recommended hardware specification.
...
* dotnet-hosting-8.0.0-win.exe (Will be downloaded from the internet if the file is not present)
...
To upgrade a base ArcGIS Enterprise deployed using the arcgis-enterprise-base deployment template to the 11.3 version, you will need:

* ArcGIS 11.3 setup archives,
* ArcGIS 11.3 software authorization files,
...
Upgrading base ArcGIS Enterprise deployments from 10.9, 10.9.1, 11.0, 11.1, or 11.2 to 11.3 requires upgrading the primary and standby machines. The file server machine does not require any changes. Steps 1 and 3 are not required for single-machine deployments.

1. Begin the upgrade on the standby machine.

   Copy attributes from the original `arcgis-enterprise-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, or 11.2 arcgis-enterprise-base template to the `arcgis-enterprise-install.json` file of the 11.3 arcgis-enterprise-base template and `arcgis-server-webadaptor-unregister.json` file of the 11.3 arcgis-webadaptor template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <11.3 JSON templates>/arcgis-enterprise-install.json
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <11.3 JSON templates>/arcgis-server-webadaptor-unregister.json
   ```
   ...
   ```shell
   chef-client -z -j <11.3 JSON templates>/arcgis-server-webadaptor-unregister.json
   ```
   ...
   ```shell
   chef-client -z -j <11.3 JSON templates>/arcgis-enterprise-install.json
   ```

2. Upgrade on the primary machine.

   Copy attributes from the original `arcgis-enterprise-primary.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, or 11.2 arcgis-enterprise-base template to the `arcgis-enterprise-primary.json` file of the 11.3 arcgis-enterprise-base template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-primary.json <11.3 JSON templates>/arcgis-enterprise-primary.json
   ```
   ...
   ```shell
   chef-client -z -j <11.3 JSON templates>/arcgis-enterprise-primary.json
   ```

3. Complete upgrading on the standby machine.

   Copy attributes from the original `arcgis-enterprise-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, or 11.2 arcgis-enterprise-base template to the `arcgis-enterprise-standby.json` file of the 11.3 arcgis-enterprise-base template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-enterprise-standby.json <11.3 JSON templates>/arcgis-enterprise-standby.json
   ```
   ...
   ```shell
   chef-client -z -j <11.3 JSON templates>/arcgis-enterprise-standby.json
   ```
