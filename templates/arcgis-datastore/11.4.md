...
Consult the ArcGIS Data Store 11.4 system requirements documentation for the required/recommended hardware specification.
...
To upgrade an ArcGIS Data Store deployed using the arcgis-datastore deployment template to the 11.4 version, you will need:

* ArcGIS Data Store 11.4 setup archive,
...
Upgrading ArcGIS Data Store deployments from 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 to 11.4 requires upgrading all ArcGIS Data Store machines. The file server machine does not require any changes.
...
1. Upgrade the standby machine.
   ...
   Copy attributes from the original `arcgis-datastore-relational-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 arcgis-datastore template to the `arcgis-datastore-install.json` and `arcgis-datastore-relational-standby.json` files of the 11.4 arcgis-datastore template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-relational-standby.json <arcgis-datastore 11.4 template>/arcgis-datastore-install.json
   
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-relational-standby.json <arcgis-datastore 11.4 template>/arcgis-datastore-relational-standby.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-datastore 11.4 template>/arcgis-datastore-install.json
   ```

   Save the updated 11.4 `arcgis-datastore-relational-standby.json` file for future upgrades from 11.4.

2. Upgrade the primary machine.

   Copy attributes from the original `arcgis-datastore-relational-primary.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 arcgis-datastore template to the `arcgis-datastore-relational-primary.json` file of the 11.4 arcgis-datastore template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-relational-primary.json <arcgis-datastore 11.4 template>/arcgis-datastore-relational-primary.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-datastore 11.4 template>/arcgis-datastore-relational-primary.json
   ```
...
1. Upgrade the standby machine.
   ...
   Copy attributes from the original `arcgis-datastore-tilecache-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 arcgis-datastore template to the `arcgis-datastore-install.json` and `arcgis-datastore-tilecache-standby.json` files of the 11.4 arcgis-datastore template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-tilecache-standby.json <arcgis-datastore 11.4 template>/arcgis-datastore-install.json
   
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-tilecache-standby.json <arcgis-datastore 11.4 template>/arcgis-datastore-tilecache-standby.json
   ```
   ...
   Save the updated 11.4 `arcgis-datastore-tilecache-standby.json` file for future upgrades from 11.4.

2. Upgrade the primary machine.

   Copy attributes from the original `arcgis-datastore-tilecache-primary.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 arcgis-datastore template to the `arcgis-datastore-tilecache-primary.json` file of the 11.4 arcgis-datastore template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-relational-primary.json <arcgis-datastore 11.4 template>/arcgis-datastore-tilecache-primary.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-datastore 11.4 template>/arcgis-datastore-tilecache-primary.json
   ```
...
1. Upgrade all machines in the cluster.
   ...
   Copy attributes from the original `arcgis-datastore-tilecache-cluster-node.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 arcgis-datastore template to the `arcgis-datastore-install.json` and `arcgis-datastore-tilecache-cluster-node.json` files of the 11.4 arcgis-datastore template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-tilecache-cluster-node.json <arcgis-datastore 11.4 template>/arcgis-datastore-install.json
   
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-tilecache-cluster-node.json <arcgis-datastore 11.4 template>/arcgis-datastore-tilecache-cluster-node.json
   ```
   ...
   Save the updated 11.4 `arcgis-datastore-tilecache-cluster-node.json` file for future upgrades from 11.4.

2: Complete the upgrade on any machine in the cluster.

   Copy attributes from the original `arcgis-datastore-tilecache-cluster.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 arcgis-datastore template to the `arcgis-datastore-tilecache-cluster.json` file of the 11.4 arcgis-datastore template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-relational-cluster.json <arcgis-datastore 11.4 template>/arcgis-datastore-tilecache-cluster.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-datastore 11.4 template>/arcgis-datastore-tilecache-cluster.json
   ```
...
1. Upgrade all machines in the cluster
   ...
   Copy attributes from the original `arcgis-datastore-spatiotemporal-node.json` JSON file created from 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 arcgis-datastore template to `arcgis-datastore-install.json` and `arcgis-datastore-spatiotemporal-node.json` of 11.4 arcgis-datastore template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-spatiotemporal-node.json <arcgis-datastore 11.4 template>/arcgis-datastore-install.json
   
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-spatiotemporal-node.json <arcgis-datastore 11.4 template>/arcgis-datastore-spatiotemporal-node.json
   ```
   ...
   Save the updated 11.4 `arcgis-datastore-spatiotemporal-node.json` file for future upgrades from 11.4.

2. Complete the upgrade on any machine in the cluster

   Copy attributes from the original `arcgis-datastore-spatiotemporal.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, or 11.3 arcgis-datastore template to the `arcgis-datastore-spatiotemporal.json` file of the 11.4 arcgis-datastore template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-datastore-spatiotemporal.json <arcgis-datastore 11.4 template>/arcgis-datastore-spatiotemporal.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-datastore 11.4 template>/arcgis-datastore-spatiotemporal.json
   ```
