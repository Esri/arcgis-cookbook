...
Consult the Portal for ArcGIS 12.0 system requirements documentation for the required/recommended hardware specification.
...
To upgrade Portal for ArcGIS deployed using the arcgis-portal deployment template to the 12.0 version, you will need:

* Portal for ArcGIS 12.0 setup archive,
* ArcGIS Web Adaptor 12.0 setup archive, if Web Adaptors were installed in the initial deployment,
* Portal for ArcGIS 12.0 software authorization file,
...
Upgrading Portal for ArcGIS deployments from 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, or 11.5 to 12.0 requires upgrading all Portal for ArcGIS machines. The file server machine does not require any changes. Step 1 can be skipped for single-machine Portal for ArcGIS deployments. In each step, wait until the Chef run completes before starting the Chef run of the next step.

1. Upgrade the Portal for ArcGIS standby machine.

   Copy attributes from the original `arcgis-portal-standby.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, or 11.5 arcgis-portal template to the `arcgis-portal-install.json` and  `arcgis-portal-standby.json` files of the 12.0 arcgis-portal template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-portal-standby.json <arcgis-portal 12.0 template>/arcgis-portal-install.json
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-portal-standby.json <arcgis-portal 12.0 template>/arcgis-portal-standby.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-portal 12.0 template>/arcgis-portal-install.json
   ```

   Save the `arcgis-portal-standby.json` file for future upgrades from 12.0 or disaster recovery.

2. Upgrade the Portal for ArcGIS primary machine.
  
   Copy attributes from the original `arcgis-portal-primary.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, or 11.5 arcgis-portal template to the `arcgis-portal-primary.json` file of the 12.0 arcgis-portal template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-portal-primary.json <arcgis-portal 12.0 template>/arcgis-portal-primary.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-portal 12.0 template>/arcgis-portal-primary.json
   ```

3. Upgrade ArcGIS Web Adaptor used with Portal for ArcGIS.

   Copy attributes from the original `arcgis-portal-webadaptor.json` JSON file created from the 10.9, 10.9.1, 11.0, 11.1, 11.2, 11.3, 11.4, or 11.5 arcgis-portal template to the `arcgis-portal-webadaptor.json` file of the 12.0 arcgis-webadaptor template.

   ```shell
   chef-apply ./templates/tools/copy_attributes.rb <original JSON files>/arcgis-portal-webadaptor.json <arcgis-webadaptor 12.0 template>/arcgis-portal-webadaptor.json
   ```
   ...
   ```shell
   chef-client -z -j <arcgis-webadaptor 12.0 template>/arcgis-portal-webadaptor.json
   ```
